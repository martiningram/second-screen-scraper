...
def get_tournament_result_links(year):
    ...
    all_result_buttons = [
        x.find("a", attrs={"class": "button-border"}) for x in all_results
    ]
...
def get_second_screen_ids(tournament_result_link):
    ...
    info = [
        {"year": x[-3], "tour_id": x[-2], "match_id": x[-1]}
...
def fetch_second_screen_json(year, tour_id, match_id):
    ...
    url = (
        f"https://www.atptour.com/-/ajax/HawkEyeSecondScreen/"
...
def fetch_match_info_json(year, tour_id, match_id):
    ...
    url = (
        f"https://www.atptour.com/-/ajax/HawkEyeSecondScreen/MatchStats/"
